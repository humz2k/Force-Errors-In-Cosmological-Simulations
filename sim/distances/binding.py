...
def cudaDist(eval_df,particle_df,precision="f8"):
    ...
    if precision == "f8":
        ...
        odds = odds.reshape((n_particles//2,n_particles))
        evens = evens.reshape((n_particles//2,n_particles))
        out = np.concatenate([evens,odds],axis=1).flatten().reshape((n_particles,n_particles))
...
def listPhis(eval_df,particle_df,precision="f2",G=1,eps=0):
    ...
    if precision == "f2":
        ...
        distances.half_precision(eval_pos_ptr,part_pos_ptr,eval_mass_ptr,G_ptr,eps_ptr,output_ptr,ctypes.c_int(n_evals),ctypes.c_int(n_particles))
