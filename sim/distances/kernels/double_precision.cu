...
__global__
void double_phis(double* eval_pos, double* part_pos, double* masses, double G, double eps, double* output, int n_particles, int n_evals){    
    ...
    if (i < n_evals){
        ...
        for (int j = 0; j < n_particles; j++){
            ...
            output[i * n_particles + j] = (G * mass) / dist;
