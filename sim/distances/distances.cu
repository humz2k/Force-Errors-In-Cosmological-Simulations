...
extern "C" {
    ...
    void double_precision_phis(double* h_eval_pos, double* h_part_pos, double* h_masses, double G, double eps, double* h_output, int n_evals, int n_particles){


        double *d_eval_pos, *d_part_pos, *d_output, *d_masses;
        ...
        cudaMalloc(&d_eval_pos,n_evals * 3 * sizeof(double));
        cudaMalloc(&d_output,n_evals * n_particles * sizeof(double));
        cudaMalloc(&d_masses,n_particles * sizeof(double));

        cudaMemcpy(d_part_pos,h_part_pos,n_particles * 3 * sizeof(double),cudaMemcpyHostToDevice);
        cudaMemcpy(d_eval_pos,h_eval_pos,n_evals * 3 * sizeof(double),cudaMemcpyHostToDevice);
        cudaMemcpy(d_masses,h_masses,n_particles * sizeof(double),cudaMemcpyHostToDevice);

        double_phis<<<numBlocks,blockSize>>>(d_eval_pos,d_part_pos,d_masses,G,eps,d_output,n_particles,n_evals);
...
extern "C" {
    ...
    void half_precision_phis(half2* h_eval_pos, half2* h_part_pos, half2* h_masses, half2* G, half2* eps, half2* h_output,int n_evals,int n_particles){
        ...
        half2 *d_eval_pos,*d_part_pos,*d_masses,*d_output;
        //half *d_eval_pos_half,*d_part_pos_half;
